...
## Backend Response Envelope (READ THIS FIRST)

NestJS backends in this stack wrap every response in a standard envelope. The service layer must **unwrap it once at the apiClient level**, so feature code never touches `success`/`data` plumbing.

### The contract

| Shape | Body |
|---|---|
| Success scalar / object | `{ "success": true, "data": <entity> }` |
| Success datatable | `{ "success": true, "data": [...], "total": N, "pageSize": N, "current": N }` |
| Error | `{ "success": false, "error": ..., "message": "...", "code"?: "..." }` |
| Error validation | `{ "success": false, "error": [...], "messages": [...], "code": "VALIDATION_ERROR" }` |

### The #1 FE bug

```typescript
// ❌ WRONG — `data` is the envelope, not the entity
const { data } = await apiClient.get('/admin/users/1');
return data.name;        // undefined — the entity is at data.data.name
```

Two fixes — pick **one** and do it consistently across the project:

**Fix A (recommended): unwrap once in axios `response` interceptor.** Feature code stays clean (`return data` always returns the entity). See `apiClient` setup below.

**Fix B: type the envelope, unwrap in each service method.** More boilerplate, but useful when you sometimes need the envelope (e.g. to read `code`).

### apiClient with envelope unwrap (Fix A)
...
type SuccessEnvelope<T> =
  | { success: true; data: T }                                                    // scalar / object
  | { success: true; data: T[]; total: number; pageSize: number; current: number }; // datatable
...
export class ApiError extends Error {
  ...
  constructor(status: number, body: ApiErrorBody) {
    super(body.message || (body.messages && body.messages.join(', ')) || 'Request failed');
    this.status = status;
    this.code = body.code;
...
// Unwrap success envelope. Datatable shape returns the full {data,total,pageSize,current}.
apiClient.interceptors.response.use(
  (res: AxiosResponse<SuccessEnvelope<unknown>>) => {
    const body = res.data;
    if (body && typeof body === 'object' && 'success' in body && body.success) {
      // Datatable: keep the meta fields
      if (Array.isArray((body as any).data) && typeof (body as any).total === 'number') {
        const dt = body as Extract<SuccessEnvelope<unknown>, { total: number }>;
        res.data = {
          data: dt.data,
          total: dt.total,
      ...
      // Scalar / object: replace res.data with the inner entity
      res.data = (body as { data: unknown }).data as any;
      return res;
    }
    return res;
  },
...
After this, services can pretend the envelope doesn't exist:

```typescript
// ✅ CORRECT after interceptor
export const usersApi = {
  getOne: async (id: string): Promise<User> => {
    const { data } = await apiClient.get<User>(`/admin/users/${id}`);
    return data;   // entity, already unwrapped
  },

  // Datatable — `data` is { data, total, pageSize, current }
  list: async (query: DatatableQuery): Promise<DatatableResult<User>> => {
    const { data } = await apiClient.post<DatatableResult<User>>(
      '/admin/users/ajax',
      query,
    );
    return data;
  ...
  bulkDelete: async (ids: string[]): Promise<{ count: number }> => {
    const { data } = await apiClient.delete<{ count: number }>('/admin/users/bulk', {
      data: { ids },
    });
    return data;
  },
...
const updateMutation = useMutation({
  ...
  onError: (err) => {
    if (err instanceof ApiError) {
      if (err.code === 'VALIDATION_ERROR' && err.messages) {
        err.messages.forEach((m) => showError(m));
        return;
      }
...
The same envelope rule applies in `fetch` calls — unwrap manually since we're not going through axios:
...
async function getUser(id: string) {
  ...
  return body.data as User;   // unwrap
}
```

For datatable on Next server side: keep `body.data`, `body.total`, `body.pageSize`, `body.current` together.

### Pitfalls

- **Don't double-unwrap.** If the apiClient interceptor unwraps, services must not also reach into `data.data`.
- **Datatable shape is special.** The interceptor returns `{ data, total, pageSize, current }` — do not destructure as `{ data }` and lose the metadata.
- **HTTP 2xx but `success: false` is impossible** with this backend (filter forces non-2xx on errors). If you ever see it, treat as a backend bug — don't add a defensive branch.
- **Empty response (204 / no body).** Interceptor short-circuits when `body` is not an envelope object; `res.data` stays `undefined`. Handle in service methods that intentionally return `void`.
...
See **Backend Response Envelope** section above for the canonical `apiClient` setup with envelope-unwrap interceptor + `ApiError` class.

**Do NOT create new axios instances** — import the shared `apiClient` so envelope unwrapping and error normalization stay consistent.
