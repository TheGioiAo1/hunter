...
**From Scratch / Complex workflows**: Dispatch `designer` agent FIRST to produce a design brief (research, user goal, tokens, layout direction, accessibility requirements). Then use the brief as input for implementation. This avoids duplicating research work inside the skill.

**Precedence:** When anti-slop rules (below) conflict with `ui-ux-pro-max` recommendations (e.g., Inter font, AI Purple palette, Lucide-only icons), substitute with alternatives from `./references/anti-slop-rules.md` unless the user explicitly requested the conflicting choice.

## Screenshot/Video Replication (Quick Reference)

1. **Analyze** - extract colors, fonts, spacing, effects
...
**Step 0 — Dispatch `designer` agent.** Before writing any code, spawn the `designer` agent to produce a design brief. The brief covers: user goal, business goal, tone/direction, token choices (colors, spacing, typography), layout per breakpoint, component inventory, states, accessibility requirements, and motion direction. Wait for the brief before proceeding.

If a design brief already exists (e.g. at `plans/reports/design-brief-*.md`), read it instead of re-dispatching.

**Step 1 — Implement from the brief.** Use the brief's tokens, layout direction, and component specs as the source of truth. Do not re-research or second-guess design decisions — the `designer` agent already validated them. If something in the brief is ambiguous, ask the user rather than improvising.

**CRITICAL**: Execute with precision. Bold maximalism and refined minimalism both work — intentionality is key. The brief sets the direction; this skill delivers the code.
...
- **Assets**: Process with `media-processing`
