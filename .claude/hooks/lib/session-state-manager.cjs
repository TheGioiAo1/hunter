...
const { resolveProjectRoot } = require('./config-utils.cjs');
...
/**
 * Resolve state dir — always anchored at the real project root (Git / CK marker),
 * never at the current working dir. This prevents writing into a sub-project's
 * `.claude/` when a subagent runs from e.g. `<root>/frontend/`.
 *
 * Fallback: global `~/.claude/session-states/{hash}/` for projects with no root marker.
 */
function getStateDir(cwd) {
  try {
    ...
    if (fs.existsSync(rootClaude)) {
      const projectDir = path.join(rootClaude, 'session-state');
